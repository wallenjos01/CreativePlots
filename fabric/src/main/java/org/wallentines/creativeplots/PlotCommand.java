...
public class PlotCommand {
    ...
    public static LiteralArgumentBuilder<CommandSourceStack> build(String id, LiteralArgumentBuilder<CommandSourceStack> builder, CommandBuildContext buildCtx, Supplier<CreativePlots> data) {
        ...
        return builder
                .then(Commands.literal("claim")
                        ...
                        .executes(ctx -> {
                            ...
                            if(map == null) {
                                ctx.getSource().sendFailure(inst.langManager.getMessage("command.error.not_plotworld", spl));
                                return 0;
                            }

                            Vec2i pos = map.getPlotPosition(spl.getBlockX(), spl.getBlockZ());
                            if(pos == null) {
                                ctx.getSource().sendFailure(inst.langManager.getMessage("command.error.not_in_plot", spl));
                                return 0;
                            }
                            ...
                            if(p == null) {
                                ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.claimed", spl));
                                return 0;
                            }
                            ...
                            ctx.getSource().sendSuccess(() -> inst.langManager.getMessageFor("command.claim", PipelineContext.builder(spl)
                ...
                .then(Commands.literal("clear")
                        ...
                        .executes(ctx -> {
                            ...
                            openConfirmGUI(ctx.getSource().getPlayerOrException(), data.get().langManager, () -> {
                                ...
                                ctx.getSource().sendSuccess(() -> data.get().langManager.getMessageFor("command.clear", PipelineContext.of(ctx.getSource().getEntity())), false);
                            });
                            return 1;
                        })
                ...
                .then(Commands.literal("delete")
                        ...
                        .executes(ctx -> {
                            ...
                            openConfirmGUI(ctx.getSource().getPlayerOrException(), data.get().langManager, () -> {
                                ...
                                ctx.getSource().sendSuccess(() -> data.get().langManager.getMessage("command.delete", ctx.getSource().getEntity()), false);
                            });
                            return 1;
                        })
                ...
                .then(Commands.literal("name")
                        .then(Commands.argument("name", StringArgumentType.greedyString())
                                ...
                                .executes(ctx -> {
                                    ...
                                    ctx.getSource().sendSuccess(() -> inst.langManager.getMessage("command.name", ctx.getSource().getEntity()), false);
                                    return 1;
                                })
                        )
                )
                .then(Commands.literal("color")
                        ...
                        .then(Commands.argument("color", StringArgumentType.greedyString())
                                .suggests((ctx, sBuilder) ->
                                        SharedSuggestionProvider.suggest(ChatFormatting.getNames(true, false), sBuilder))
                                .executes(ctx -> {
                                    ...
                                    CreativePlots inst = data.get();
                                    DataResult<TextColor> res = TextColor.parseColor(StringArgumentType.getString(ctx, "color"));
                                    if(res.isError()) {
                                        ctx.getSource().sendFailure(inst.langManager.getMessage("command.error.invalid_color", ctx.getSource().getEntity()));
                                        return 0;
                                    }
                                    ...
                                    ctx.getSource().sendSuccess(() -> inst.langManager.getMessage("command.color", ctx.getSource().getEntity()), false);
                                    return 1;
                                })
                        )
                )
                .then(Commands.literal("editor")
                        ...
                        .then(Commands.literal("add")
                                .then(Commands.argument("name", StringArgumentType.greedyString())
                                        .suggests((ctx, sBuilder) ->
                                                SharedSuggestionProvider.suggest(ctx.getSource().getServer().getPlayerNames(), sBuilder))
                                        .executes(ctx -> {
                                            Plot p = getOwnedPlot(ctx);
                                            if(p == null) return 0;
                                            ...
                                            getUUID(ctx).thenAccept(uuid -> {
                                                if(uuid == null) {
                                                    ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.invalid_player", ctx.getSource().getEntity()));
                                                    return;
                                                }
                                                p.editors().add(uuid);
                                                ctx.getSource().sendSuccess(() -> data.get().langManager.getMessage("command.editor.add", ctx.getSource().getEntity()), false);
                                            });
                        ...
                        .then(Commands.literal("remove")
                                .then(Commands.argument("name", StringArgumentType.greedyString())
                                        .suggests((ctx, sBuilder) -> {
                                            ...
                                            return SharedSuggestionProvider.suggest(p.editors().stream().map(uuid ->
                                                    ctx.getSource().getServer().getProfileCache().get(uuid).map(GameProfile::getName).orElse(uuid.toString())), sBuilder);
                                        })
                                        .executes(ctx -> {
                                            Plot p = getOwnedPlot(ctx);
                                            if(p == null) return 0;
                                            ...
                                            getUUID(ctx).thenAccept(uuid -> {
                                                ...
                                                ctx.getSource().sendSuccess(() -> data.get().langManager.getMessage("command.editor.remove", ctx.getSource().getEntity()), false);
                                            });
                                ...
                                .executes(ctx -> {
                                    ...
                                    getUUID(ctx).thenAccept(uuid -> {
                                        ...
                                        openConfirmGUI(spl, data.get().langManager, () -> {
                                            p.setOwner(uuid);
                                            ctx.getSource().sendSuccess(() -> data.get().langManager.getMessage("command.transfer", ctx.getSource().getEntity()), false);
                                        });
    ...
    private CompletableFuture<UUID> getUUID(CommandContext<CommandSourceStack> ctx) {
        ...
        if(userPlayer == null) {
            return new ResolvableProfile(Optional.of(username), Optional.empty(), new PropertyMap()).resolve().thenApply(res -> res.gameProfile().getId());
        } else {
    ...
    private Plot getOwnedPlot(CommandContext<CommandSourceStack> ctx) throws CommandSyntaxException {
        ...
        if(p.p1 == null) {
            ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.not_plotworld", ctx.getSource().getEntity()));
            return null;
        }

        if(p.p2 == null) {
            if(p.p1 == PlotMap.BlockType.PLOT) {
                ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.not_in_plot", ctx.getSource().getEntity()));
            } else {
                ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.not_claimed", ctx.getSource().getEntity()));
            }
            return null;
        }

        if(!p.p2.hasOwnerPermissions(spl)) {
            ctx.getSource().sendFailure(data.get().langManager.getMessage("command.error.not_owner", ctx.getSource().getEntity()));
            return null;
        }
    ...
    private static void openConfirmGUI(ServerPlayer spl, LangManager<?, Component> langManager, Runnable onClick) {
        ...
        gui.setItem(5, ctx -> new ItemStack(Holder.direct(Items.LIME_STAINED_GLASS_PANE), 1, DataComponentPatch.builder()
